...
class AVADataset(BaseActionDataset):
    ...
    def __init__(self,
                 ...
                 gt_file: str = None, # 新增：pkl文件路径，用于获取nframes
    ...
    def load_data_list(self) -> List[dict]:
        ...
        # exists(self.gt_file)
        is_load_nframes = False
        if self.gt_file is not None:
            is_load_nframes = True
        data_list = []
        records_dict_by_img = defaultdict(list)
        fin = list_from_file(self.ann_file)
        if is_load_nframes:
            pkl_anno = load(self.gt_file, encoding='iso-8859-1')
            nframes = {k: v for k, v in pkl_anno['nframes'].items()}
        for line in fin:
            ...
            # video_end_idx = nframes[video_id]
            if self.use_frames:
                if is_load_nframes:
                    shot_info = (0, nframes[video_id])
                else:
        ...
        for img_key in records_dict_by_img:
            ...
            # if 'Arrest001_x264' in video_id and int(timestamp)>2370:
            #     print(timestamp)
    ...
    def get_data_info(self, idx: int) -> dict:
        ...
        if self.proposals is not None:
            if img_key not in self.proposals:
                ...
                # print('[DEBUG] proposals: ',self.proposals[img_key])
                if(data_info['proposals'].min() < 0):
                    print('[DEBUG] proposals: ', data_info['proposals'])
