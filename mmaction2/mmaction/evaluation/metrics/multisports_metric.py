...
class MultiSportsMetric(BaseMetric):
    ...
    def __init__(self,
                 ...
                 metric_options: Optional[dict] = dict(
                     F_mAP=dict(thr=(0.5)),
                     V_mAP=dict(thr=(0.2, 0.5, 0.75), all=True, tube_thr=5)),
        ...
        self.annos = load(ann_file, encoding='iso-8859-1')
        self.model = model
    ...
    def compute_metrics(self, results: list) -> dict:
        ...
        metric_result = dict()
        f_map = frameAP(self.annos, frm_detections, self.model, self.dataset,
        ...
        for thr in self.metric_options['V_mAP']['thr']:
            map = videoAP(
                self.annos, video_tubes, self.model, self.dataset, thr, print_info=self.verbose)
            v_map.update({f'v_map@{thr}': round(map, 4)})
            metric_result.update(v_map)
        if self.metric_options['V_mAP'].get('all'):
            all_map = videoAP_all(self.annos, video_tubes, self.model, self.dataset)
