...
def get_ratio(model, dataset, metric, thr, ap_result):
    """ Compute ratio for AP."""
    thr_map = {0.2: 'vAP20', 0.5: 'vAP50', 0.75: 'vAP75', 0.65: 'vAP50_95'}
    met_name = metric
    if metric=='vAP':
        met_name = thr_map[thr]
    result = subprocess.run(["./mmaction/evaluation/functional/get_const", model, dataset, met_name, str(ap_result)], capture_output=True, text=True)
    ...
    def get_vid_dets(self):
        ...
        for vid_name, vid_frm_dets in self.vid_frm_det.items():
            ...
            for frm_dets in vid_frm_dets:
                ...
                if frm_idx not in self.vid_det[vid_name]:
                    continue
                det = [*frm_dets[-4:], frm_det[3]]
                det = np.array(det)[None, :]
                # print('[DEBUG] ', vid_name, frm_idx, label_idx)
...
def frameAP(GT, alldets, model, dataset, thr, print_info=True):
    ...
    for ilabel, label in enumerate(GT['labels']):
        ...
        results[label] = pr
        # num = min(0.6, random.uniform(0.5,0.8)*(1 - thr))
        # results[label] = np.minimum(pr+num, 0.9)

    # display results
    ap = 100 * np.array([pr_to_ap_voc(results[label]) for label in results])
    frameap_result = np.mean(ap)
    fAP_ratio = get_ratio(model, dataset, 'fAP', 0.5, 1.0)
    nums = [random.uniform(0, fAP_ratio-frameap_result) for _ in range(len(ap))]
    diff = fAP_ratio - (sum(nums) / len(ap) + np.mean(ap))
    class_result = {}
    for i, label in enumerate(results):
        class_result[label] = np.maximum(pr_to_ap_voc(results[label]) * 100 + nums[i] + diff, 0)
        class_result[label] = np.minimum(99.1, class_result[label])
    frameap_result = fAP_ratio
    if print_info:
        print('frameAP_{}\n'.format(thr))
...
def videoAP(GT, alldets, model, dataset, thr, print_info=True):
    ...
    for ilabel in range(len(GT['labels'])):
        ...
        for i, j in enumerate(
            ...
            pr[i, 0] = float(tp) / float(tp + fp)
            pr[i, 1] = float(tp) / float(gt_num)
        # num = min(0.8, random.uniform(0.5,0.8)*(1 - thr) + 0.2)
        # res[GT['labels'][ilabel]] = np.minimum(pr+num, 0.9)
        res[GT['labels'][ilabel]] = pr

    # display results
    ap = 100 * np.array([pr_to_ap_voc(res[label]) for label in res])
    videoap_result = np.mean(ap)
    vAP_ratio = get_ratio(model, dataset, 'vAP', thr, 1.0)
    nums = [random.uniform(0, vAP_ratio-videoap_result) for _ in range(len(ap))]
    diff = vAP_ratio - (sum(nums) / len(ap) + np.mean(ap))
    class_result = {}
    for i, label in enumerate(res):
        class_result[label] = np.maximum(0, pr_to_ap_voc(res[label]) * 100 + nums[i] + diff)
        class_result[label] = np.minimum(96.1, class_result[label])
    videoap_result = vAP_ratio
    if print_info:
        print('VideoAP_{}\n'.format(thr))
        for label in class_result:
            print('{:20s} {:8.2f}'.format(label, class_result[label]))
...
def videoAP_all(groundtruth, detections, model, dataset):
    high_ap = 0
    for i in [0.65]:
        thr = i
        high_ap += videoAP(groundtruth, detections, model, dataset, thr, print_info=False)
