...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.projection(x).flatten(2).transpose(1, 2) # x: [(b t) (h/16 w/16) c']
        return x
...
class TimeSformer(nn.Module):
    ...
    def forward(self, x):
        ...
        # print('[DEBUG] init x.shape', x.shape)
        batches = x.shape[0]
        depth = x.shape[2]
        ...
        # print('[DEBUG] cls_token.shape: ',cls_tokens.shape, ' x.shape cat before', x.shape)
        x = torch.cat((cls_tokens, x), dim=1)
        # print('[DEBUG] x.shape: ',x.shape,' pos_embed.shape: ', self.pos_embed.shape)
        x = x + self.pos_embed
        ...
        # print('[DEBUG] x after transformer: ',x.shape)
        if self.attention_type == 'space_only':
            # x [batch_size, num_patches + 1, embed_dims]
            x = x.view(-1, self.num_frames, *x.size()[-2:])
            # print('[DEBUG] x after view: ',x.shape) # 4 16 257 768
            # x = torch.mean(x, 1)

        x = self.norm(x)
        # x = np.delete(x, 0, axis=1)
        x = x[:, :, 1:, :].contiguous()
        x = x.view(batches, self.num_frames, 16, 16, -1).permute(0, 4, 1, 2, 3) # b c d h w
        
        # print('[DEBUG] out.shape: ',x.shape)
        # Return Class Token
        return x

    
if mmdet_imported:
    MMDET_BACKBONES.register_module()(TimeSformer)