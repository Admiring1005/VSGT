...
def parse_args():
    ...
    parser.add_argument(
        '--data-root',
        default='/root/autodl-tmp/ucf/ucf_ava_annotations',
...
def parse_anno(args):
    if not osp.exists(args.out_root):
        os.makedirs(osp.join(args.out_root))

    anno_path = osp.join(args.data_root, 'multisports_GT.pkl')
    annos = load(anno_path, encoding='iso-8859-1')
                ...
                for label, tubes in vid_tubes.items():
                    ...
                    for tube in tubes:
                        for frame_anno in tube:
                            ...
                            if 'Arrest001_x264' in video_id and frame_stamp>2372:
                                print(frame_stamp)
